...
export async function render() {
  try {
    ...
    // Tier Logic based on ARC volume
      ...
      <div class="grid-2" style="margin-bottom: 24px;">
        <div class="card" style="margin-bottom: 0;">
          ...
          <div style="display: flex; justify-content: space-between; font-size: 13px;">
            <span>${formatToken(totalVolume)} ARC processed</span>
            <span style="color: var(--text-muted)">${currentTier === 'Scale' ? 'Max Tier' : `Next tier at ${nextThreshold} ARC`}</span>
          </div>
        ...
        <div class="stat-card" style="display: flex; flex-direction: column; justify-content: center;">
          ...
          <div class="stat-value" style="color: var(--accent-warning); font-size: 48px;">
            ${formatToken(stats.feesEarned)} <span style="font-size: 16px; color: var(--text-muted);">ARC</span>
          </div>
